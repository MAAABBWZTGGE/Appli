...
public class Cross {
	...
	public void update() {
		if (cross > -1) {
			double crossDist = Math.sqrt(Math.pow(crossX - sim.getCoord(0),
					2)
			...
			double pDist = Math.sqrt(Math.pow(pDx, 2) + Math.pow(pDy, 2)
					+ Math.pow(pDz, 2));
			...
			crossX = sim.getPathCoord(cross, crossInd, 0) + pDx * crossPos
					/ pDist;
			crossY = sim.getPathCoord(cross, crossInd, 1) + pDy * crossPos
					/ pDist;
			crossZ = sim.getPathCoord(cross, crossInd, 2) + pDz * crossPos
					/ pDist;
	...
	public double getCoord(int c) {
		if (c==0) {return crossX;}
		if (c==1) {return crossY;}
		return crossZ;
	}
}
