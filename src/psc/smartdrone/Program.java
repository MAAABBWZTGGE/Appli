...
public class Program {
	...
	public void mainLoop() {
		dt = (System.currentTimeMillis()-time)/1000;
		if (dt<rate) return;
		time = System.currentTimeMillis();
		...
		Vector3 dir = new Vector3(Math.cos((psi+90)*(Math.PI)/180),Math.sin((psi+90)*(Math.PI)/180),0);
		Vector3 ldir = new Vector3(0,Math.cos((theta+270)*(Math.PI)/180),Math.sin((theta+270)*(Math.PI)/180));
		Vector3 hdir = new Vector3(Math.cos((phi+90)*(Math.PI)/180),0,Math.sin((phi+90)*(Math.PI)/180));
		...
		if (-sim.getCoord(2)<=8) {
		pd(0,dir, new Vector3(cross.getCoord(0)-X,cross.getCoord(1)-Y,cross.getCoord(2)-Z),2,kp,kd,ki);}
		
		double kRot = 0.1;
		Vector3 proj = new Vector3(-cross.getCoord(0)+X,-cross.getCoord(1)+Y,0);
		double thetaCons=(180/Math.PI)*Math.atan(2*speed*kRot*dir.Scalar(proj)/(9.81*dir.Norm()*proj.Norm()));

		pd(1,ldir,new Vector3(0,Math.cos(-thetaCons*(Math.PI)/180),Math.sin(-thetaCons*(Math.PI)/180)),0,30,0.0005,1);
		
		System.out.println(speed);
		pd(2,hdir,new Vector3(Math.sqrt(Math.pow(-cross.getCoord(0)+X,2)+Math.pow(-cross.getCoord(1)+Y,2)),0,cross.getCoord(2)+Z),1,kp,kd,ki);
